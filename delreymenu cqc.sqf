 ...
 _closeebutton ctrlSetPosition [-0.1275,-0.0624,0.1208032,0.0285586];
  ...
  findDisplay 0 displayCtrl 999 setVariable ['_TARGETINFOXDAWD', dahwduiahwduihiuawhdiuhaiuwduiahwdiuanwdiubnauy8wbdunaiw];
      ...
      _ewoirhouihdsfiusuidfuisdbuifw5 = [
            ["DELREY Private CQC Version","DRed"],
            ["DELREY.FAMILY","LRed"],
            ["","DRed"],
            ...
            ["","LRed"],
            ["PESP BOX ALL","DRed"],
            ["PESP BOX BLUEFOR","LRed"],
            ["PESP BOX OPFOR","DRed"],
            ["PESP BOX INDEPENDENT","LRed"],
            ["PESP BOX CIVILIAN","DRed"],
            ["","LRed"],
			      ["Aim assist","DRed"],
            ["No recoil","LRed"],
			      ["Unlimited ammo","DRed"],
...
switch (_0FUCKS) do
{
  ...
  if (!isNull _target) then {
    ...
    if ((alive _target) && (_target isKindOf "Man") && (group _target != group player)) then {
          drawIcon3D ["", [1,0,0,1], position cursorTarget, 0, 0, 0, "", 1, 0.04, "PuristaMedium"];
    };
  };
}];
                            ...
                            if (player distance _selectedunit <= 4000) then {
                                if (_selectedunit in (units group player)) then {
                                  ...
                                  if (side _selectedunit == west) then {
                                      ...
                                      drawIcon3D["", [0.082,0.235,0.847,1], _poz, 0, -1.9, 0, (format["0", _name, (round(player distance _selectedunit))]), 1.5, 0.10, "PuristaLight", "center", true];
                                  };
                                  if (side _selectedunit == east) then {
                                      drawIcon3D["", [0.706,0,0,1], _poz1, 0, -1, 0, (format["%1 (%2m)", _name, (round(player distance _selectedunit))]), 1.5, 0.03, "PuristaLight", "center", true];
